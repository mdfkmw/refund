...
export default function AdminOnlineSettings() {
    ...
    fetch('/api/online-settings', { credentials: 'include' })
      ...
      .then((data) => {
        ...
        const normalized = {
          ...
          onlineCancelMinNoticeMinutes: Number(data?.onlineCancelMinNoticeMinutes) || 0,
          onlineModifyMinNoticeMinutes: Number(data?.onlineModifyMinNoticeMinutes) || 0,
        };
  ...
  const isDirty = useMemo(() => {
    return (
      ...
      form.onlineCancelMinNoticeMinutes !== initialValues.onlineCancelMinNoticeMinutes ||
      form.onlineModifyMinNoticeMinutes !== initialValues.onlineModifyMinNoticeMinutes
    );
  }, [form, initialValues]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const normalized = {
        ...
        onlineCancelMinNoticeMinutes: Number(settings.onlineCancelMinNoticeMinutes) || 0,
        onlineModifyMinNoticeMinutes: Number(settings.onlineModifyMinNoticeMinutes) || 0,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block font-medium">
              Anulare rezervare online
            </label>
            <p className="text-sm text-gray-600 mb-2">
              Numărul minim de minute înainte de plecare necesar pentru anulare.
            </p>
            <div className="flex items-center gap-3">
              <input
                type="number"
                min={0}
                max={20160}
                value={form.onlineCancelMinNoticeMinutes}
                onChange={handleNumberChange('onlineCancelMinNoticeMinutes')}
                className="w-32 rounded border border-gray-300 px-2 py-1"
              />
              <span className="text-sm text-gray-500">
                {minutesToLabel(form.onlineCancelMinNoticeMinutes)}
              </span>
            </div>
          </div>
          ...
          <div>
            <label className="block font-medium">
              Modificare rezervare online
            </label>
            <p className="text-sm text-gray-600 mb-2">
              Numărul minim de minute înainte de plecare necesar pentru modificare.
            </p>
            <div className="flex items-center gap-3">
              <input
                type="number"
                min={0}
                max={20160}
                value={form.onlineModifyMinNoticeMinutes}
                onChange={handleNumberChange('onlineModifyMinNoticeMinutes')}
                className="w-32 rounded border border-gray-300 px-2 py-1"
              />
              <span className="text-sm text-gray-500">
                {minutesToLabel(form.onlineModifyMinNoticeMinutes)}
              </span>
            </div>
          </div>
