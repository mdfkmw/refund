...
useEffect(() => {
  ...
  try {
    ...
    if (!res.ok) throw new Error('Nu am putut încărca terminalele');
    const data = await res.json();
    ...
    setInviteForm(f => ({
      ...f,
      default_terminal_id: f.default_terminal_id ?? firstId,
    }));
  } catch (err) {
    console.error('fetchTerminals failed', err);
    setTerminals([]);
  }
};
...
const payload = {
  ...
  default_terminal_id: inviteForm.default_terminal_id ? Number(inviteForm.default_terminal_id) : null,
};
    ...
    <div>
      ...
      <section className="mt-10 max-w-xl">
        ...
        <form onSubmit={createInvitation} className="space-y-4 bg-gray-50 border border-gray-200 rounded p-4">
          ...
          {terminals.length > 0 && inviteForm.role === 'agent' && (
  <label className="block text-sm font-medium">
    Terminal default (pentru agent)
    <select
      className="mt-1 w-full border rounded px-3 py-2 text-sm"
      value={inviteForm.default_terminal_id ?? ''}
      onChange={e => setInviteForm(f => ({ ...f, default_terminal_id: e.target.value || null }))}
      required
    >
      {terminals.map(t => (
