...
const ONLINE_SETTING_KEYS = {
  ...
  onlineCancelMinNoticeMinutes: 'online_cancel_min_notice_minutes',
  onlineModifyMinNoticeMinutes: 'online_modify_min_notice_minutes',
};

const DEFAULT_ONLINE_SETTINGS = {
  blockPastReservations: false,
  publicMinNoticeMinutes: 0,
  publicMaxAdvanceMinutes: 0,

  onlineCancelMinNoticeMinutes: 0,
  onlineModifyMinNoticeMinutes: 0,
};
...
async function getOnlineSettings() {
  ...
  return {
    ...
    onlineCancelMinNoticeMinutes: parseNonNegativeInteger(
      map.get(ONLINE_SETTING_KEYS.onlineCancelMinNoticeMinutes),
      DEFAULT_ONLINE_SETTINGS.onlineCancelMinNoticeMinutes,
    ),
    onlineModifyMinNoticeMinutes: parseNonNegativeInteger(
      map.get(ONLINE_SETTING_KEYS.onlineModifyMinNoticeMinutes),
      DEFAULT_ONLINE_SETTINGS.onlineModifyMinNoticeMinutes,
    ),
  };
}
...
async function saveOnlineSettings(settings) {
  const payload = {
    ...
    onlineCancelMinNoticeMinutes: String(parseNonNegativeInteger(
      settings.onlineCancelMinNoticeMinutes,
      DEFAULT_ONLINE_SETTINGS.onlineCancelMinNoticeMinutes,
    )),
    onlineModifyMinNoticeMinutes: String(parseNonNegativeInteger(
      settings.onlineModifyMinNoticeMinutes,
      DEFAULT_ONLINE_SETTINGS.onlineModifyMinNoticeMinutes,
    )),
  };
  ...
  await Promise.all([
    ...
    setAppSetting(ONLINE_SETTING_KEYS.onlineCancelMinNoticeMinutes, payload.onlineCancelMinNoticeMinutes),
    setAppSetting(ONLINE_SETTING_KEYS.onlineModifyMinNoticeMinutes, payload.onlineModifyMinNoticeMinutes),
  ...
  return {
    ...
    onlineCancelMinNoticeMinutes: Number(payload.onlineCancelMinNoticeMinutes),
    onlineModifyMinNoticeMinutes: Number(payload.onlineModifyMinNoticeMinutes),
  };
}
