...
// Dacă un agent_job rămâne blocat in_progress mai mult de X minute, îl marcăm error
const AGENT_JOB_STUCK_MINUTES = Number(process.env.AGENT_JOB_STUCK_MINUTES || 10);
...
async function runCleanup(reason = "interval") {
  ...
  try {
    ...
    // 5) Cleanup: agent_jobs blocate in_progress (agentul a picat, nu a mai trimis report)
    const markStuckAgentJobs = await db.query(
      `
  UPDATE agent_jobs
  SET status = 'error',
      error_message = CONCAT('timeout: stuck in_progress > ', ?, ' minutes')
  WHERE status = 'in_progress'
    AND updated_at < (NOW() - INTERVAL ? MINUTE)
  `,
      [AGENT_JOB_STUCK_MINUTES, AGENT_JOB_STUCK_MINUTES]
    );
...
console.log(
  `${tag} OK: expiredOrders=${expireOrders.rowCount || 0}, ` +
    ...
    `stuckAgentJobs=${markStuckAgentJobs.rowCount || 0}`
);
