...
router.put('/', requireAuth, requireRole('admin', 'operator_admin'), async (req, res) => {
  try {
    ...
    const cancelMinNotice = clampNumber(body.onlineCancelMinNoticeMinutes, {
      min: 0,
      max: 60 * 24 * 14,
      fallback: 0,
    });

    const modifyMinNotice = clampNumber(body.onlineModifyMinNoticeMinutes, {
      min: 0,
      max: 60 * 24 * 14,
      fallback: 0,
    });
...
const saved = await saveOnlineSettings({
  ...
  onlineCancelMinNoticeMinutes: cancelMinNotice,
  onlineModifyMinNoticeMinutes: modifyMinNotice,
});
