...
router.post('/', requireRole('admin','operator_admin'), async (req, res) => {
  const { role, operator_id = null, email, ttl_hours = 72, default_terminal_id = null } = req.body || {};

  if (!role || !email) return res.status(400).json({ error: 'role și email obligatorii' });

  const token = genToken();
await q(
  `INSERT INTO invitations (token, role, operator_id, email, default_terminal_id, expires_at, created_by)
   VALUES (?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? HOUR), ?)`,
  [token, role, operator_id || null, email, default_terminal_id, ttl_hours, req.user?.id || null]
);
...
router.post('/accept', async (req, res) => {
  ...
  // creăm employee
await q(
  `INSERT INTO employees (name, username, email, role, operator_id, default_terminal_id, active, password_hash)
   VALUES (?, ?, ?, ?, ?, ?, 1, ?)`,
  [name, trimmedUsername, inv.email, inv.role, inv.operator_id || 1, inv.default_terminal_id || null, pass]
);
